...
export const App: FC = () => {
  ...
  useNuiEvent<boolean>(resourceName, "showUI", (r) =>
    dispatch(setIsAppVisible(r))
  );

  useNuiEvent<Screen[]>(resourceName, "getScreenList", (r) =>
    dispatch(setScreens(r))
  );
  ...
  useNuiEvent<Schedule>(resourceName, "createSchedule", (r) => {
    showSnackbar(`Schedule successfully created!`);
    navigate("/schedules", { replace: true });
  });

  useNuiEvent<Schedule>(resourceName, "editSchedule", (r) =>
    showSnackbar(`Schedule successfully updated!`)
  );

  useNuiEvent<string>(resourceName, "deleteSchedule", (r) =>
    showSnackbar(`Schedule successfully deleted`)
  );

  useNuiEvent<Schedule[]>(resourceName, "getScheduleList", (r) =>
    dispatch(setSchedules(r))
  );

  const showSnackbar = (
    message: string,
    type: SnackbarSeverity = SnackbarSeverity.Success
  ) => {
    dispatch(
      setSnackbar({
        isVisible: true,
  ...
  return (
    <Draggable handle="#appHeader" nodeRef={nodeRef}>
      <Wrapper
        ref={nodeRef}
        style={{
          marginTop: "80px",
